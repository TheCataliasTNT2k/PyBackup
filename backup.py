...
from config_backup import Config, create_from_json
...
def docker_db_backup(container: Container, username: str, password: str, database: str, backup_dir: str) -> list:
    ...
    if os.path.exists(path):
        ...
        return [database, " ", f"{Fore.YELLOW}Skipped{Fore.RESET}"]
    ...
    return [
        ...
        f"{Fore.GREEN}OK{Fore.RESET}" if res.exit_code == 0 else f"{Fore.RED}Failed ({res.exit_code}){Fore.RESET}",
    ]
...
def main(config: Config):
    ...
    for job in jobs:
        if job == "database":
            try:
                container = list(
                    filter(lambda c: c.name == config.database_container_name, docker_env().containers.list())
                )[0]
            except IndexError:
            ...
            for database in config.database_list:
                data.append(
                    docker_db_backup(
                        container, config.database_username, config.database_password, database, config.backup_dir
                    )
                )
            ...
            with open(f"{config_object.backup_dir}/{method}sum.txt", "a") as f:
                for filename in os.listdir(config_object.backup_dir):
                    if filename in files_to_skip:
                        continue
                    val = getattr(hashlib, method)(open(f"{config_object.backup_dir}/{filename}", "rb").read()).hexdigest()
